...
kas_sites_sheets <- 
  map(kas_sites_names[1:4], ~kas_sites %>% gs_read_listfeed(ws = .x))
```
...
names(kas_sites_sheets) <- kas_sites_names[1:4]

# convert list of dataframes into one big dataframe
kas_sites_sheets_altogether <- 
kas_sites_sheets %>% 
  # we need to make the report_id as character, since some are integer
  map(~.x %>% mutate(report_id = as.character(report_id),
                     layer = as.character(layer))) %>% 
  bind_rows(.id = 'site')

# make a new col with recode artefact type as retouch and non-retouch
kas_sites_sheets_altogether <- 
  kas_sites_sheets_altogether %>% 
  mutate(retouch = ifelse(name %in% c("side_scraper",  
                                      "end_scraper", 
                                      "notch",  
                                      "awl", 
                                      "stemmed point",
...
# frequency of retouched pieces per site & density
kas_sites_sheets_altogether_density <- 
kas_sites_sheets_altogether %>% 
  mutate(sites = as.character(glue('{site}_{layer}'))) %>% 
  mutate(sites = str_replace_all(sites, "_NA", "")) %>% 
  group_by(sites, retouch) %>% 
  tally() %>% 
  spread(retouch, n) %>% 
  mutate(proportion_retouched = retouched / `non-retouched`) %>% 
  # join with density data
  left_join(kasv_tidy, by = c('sites' = 'sites'))

# compute correlation
kas_sites_retouch_density_corr <- 
cor.test(kas_sites_sheets_altogether_density$retouched,
         kas_sites_sheets_altogether_density$artefact_density)
...
ggplot(kas_sites_sheets_altogether_density,
       aes(retouched,
           artefact_density 
           )) +
  geom_point(size = 4,
             aes(colour = date_age / 1000)) +
  geom_text_repel(aes(label = sites),
                  nudge_x = 0.075, 
                  nudge_y = -0.075,
                  size = 5) +
  ...
  ylab("Frequency of retouched pieces") +
  annotate("text",
           x = 4,
           y = 3,
           label = "Curated",
...
<<<<<<< HEAD
The artefact volumetric density is defied as the total number of artefacts per cubic meter of excavated sediment, and served as a proxy for accumulation ratio of artefacts. the relative frequency of retouched pieces is the count of retouched flake tools devided by the total number of artifacts. 
...
=======
>>>>>>> a2660d24190c122c94133893fc4cbdde171c8378
